...
```bash
Week9-API$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
Week9-API$ sam local start-api
Week9-API$ curl http://localhost:3000/classify_digit
```
...
```bash
Week9-API$ sam logs -n InferenceFunction --stack-name Week9-API --tail
```
...
```bash
aws cloudformation delete-stack --stack-name Week9-API
```
